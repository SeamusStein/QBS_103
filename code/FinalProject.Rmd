...
#annotation_colors = list(sex = c(female="blue", male="green"), icu_status = c(no="yellow", yes = "purple"))
# define the annotation colors
annotation_colors <- list('ICU Status' = c('no' = 'green',
                                   'yes' = 'red'))
#my_colors = list(
 #   Sex = c(female = "#5977ff", male = "#f74747")
#)

# plot heatmap of 10 genes  
pheatmap(gene_heatmap_data,
...
make_raincloud <- function(df, gene){
   # create label pieces 
    ...
    my_y <- eval(bquote(expression(~ italic(.(gene_label[1])) ~ .(labNames[1]))))
